...
if (isset($_GET['edit'])) {
    ...
    echo "ID do fiscal não especificado.";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "UPDATE fiscais SET nome = ?, email = ?, telefone = ?, cpf = ?, senha = ?, foto = ? WHERE fiscal_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ssssssi', $nome, $email, $telefone, $cpf, $senha, $foto, $fiscal_id);
    
    if ($stmt->execute()) {
        header("Location: gerenfis.php?message=Fiscal editado com sucesso");
        exit;
    } else {
        echo "Erro ao editar fiscal: " . $stmt->error;
    }
}
?>
...
<header>
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4" style="margin-left: 250px; padding-top: 30px;">
        ...
        <form method="post" enctype="multipart/form-data">
            <input type="hidden" name="fiscal_id" value="<?php echo $fiscal['fiscal_id']; ?>">
            <div>
                <label>Nome:</label>
                <input type="text" name="nome" value="<?php echo $fiscal['nome']; ?>" required>
            </div>
            <div>
                <label>Email:</label>
                <input type="email" name="email" value="<?php echo $fiscal['email']; ?>" required>
            </div>
            <div>
            <div class="input-group">
            <div>
                <label for="">Telefone:</label>
                <input type="text" name="telefone" value="<?php echo $fiscal['telefone']; ?>"><br>
            </div>
            <div>
                <label for="">CPF:</label>
                <input type="text" name="cpf" value="<?php echo $fiscal['cpf']; ?>"><br>
            </div>
            ...
            <a href="gerenfis.php" class="btnproj">Voltar</a>
            </div>
        </form>
    </div>
    </main>
